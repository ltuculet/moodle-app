...
def debug_get_my_courses_page():
    ...
    if not moodle_base_url:
        ...
        print("APP.PY ERROR (DEBUG_ROUTE): URL de Moodle no configurada.")
        return redirect(url_for('config_moodle_url'))

    if not session.get('moodle_user_active') or 'moodle_cookies' not in session:
        flash('No has iniciado sesión en Moodle. Por favor, inicia sesión primero.', 'warning')
        print("APP.PY ERROR (DEBUG_ROUTE): Usuario no logueado o cookies no encontradas.")
        return redirect(url_for('login_moodle'))
    ...
    if not req_session:
        ...
        print("APP.PY ERROR (DEBUG_ROUTE): _get_moodle_session_from_flask_session devolvió None.")
        return redirect(url_for('login_moodle'))

    if not _is_moodle_session_still_valid(req_session, moodle_base_url):
         flash('La sesión de Moodle parece haber expirado o es inválida. Por favor, re-logueate.', 'warning')
         print("APP.PY WARN (DEBUG_ROUTE): Sesión de Moodle inválida detectada.")
         session.pop('moodle_user_active', None)
         session.pop('moodle_cookies', None)
         return redirect(url_for('login_moodle'))
    ...
    debug_target_url = ""
    try:
        debug_target_url = moodle_client.get_my_courses_url(moodle_base_url)
    except Exception as e_url_helper:
        print(f"APP.PY ERROR (DEBUG_ROUTE): Error llamando a moodle_client.get_my_courses_url: {e_url_helper}")
        flash(f"Error interno obteniendo URL de cursos: {e_url_helper}", "danger")
        return redirect(url_for('dashboard'))

    print(f"APP.PY DEBUG (RUTA /debug_get_my_courses_page): Accediendo a {debug_target_url}")
    try:
        debug_response = req_session.get(debug_target_url, timeout=20)
        ...
        flash(f"HTML de {debug_target_url} guardado en el servidor como '{output_filename}'. Por favor, revisa la carpeta del proyecto.", 'success')
        return redirect(url_for('dashboard'))

    except requests.exceptions.Timeout:
        msg = f"Timeout al acceder a {debug_target_url}"
        print(f"APP.PY ERROR (RUTA /debug_get_my_courses_page): {msg}")
        flash(msg, 'danger')
        return redirect(url_for('dashboard'))
    except requests.exceptions.RequestException as e_req_debug:
        msg = f"Error de red/HTTP al acceder a {debug_target_url}: {e_req_debug}"
        print(f"APP.PY ERROR (RUTA /debug_get_my_courses_page): {msg}")
        flash(msg, 'danger')
        return redirect(url_for('dashboard'))
    except Exception as e_debug_save:
        msg = f"Error al guardar HTML desde {debug_target_url}: {e_debug_save}"
        print(f"APP.PY ERROR (RUTA /debug_get_my_courses_page): {msg}")
        flash(msg, 'danger')
        return redirect(url_for('dashboard'))
